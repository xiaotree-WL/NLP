...
public class TimeNormalizer implements Serializable {
    ...
    /**
     * 这里对一些不规范的表达输入做转换,对于下个周末这种做转化 把个给移除掉
     * @param inputString
     * @return
     */
    public static String filterUnstandard(String inputString) {
        inputString = StringPreHandlingModule.numberTranslator(inputString);// 大写数字转化一六零加一五八可以转化为160+158
        String[] temp = inputString.split("[。|，|,|?|？| ]");
        StringBuffer stringBuffer = new StringBuffer();
        boolean flag = false;
        String ruleA = "(没有?[时|空])|((?<!行)不[行|用|能](?!就))|((?<!没)有[^时]*[家|面|会|事|约|安])|([约|满].?了)";
        Pattern patternA = Pattern.compile(ruleA);
        for (int i = 0; i < temp.length; i++) {
            if (patternA.matcher(temp[i]).find() || temp[i].contains("忙") || temp[i].contains("休") || temp[i].contains("工作时间")
                    || temp[i].contains("已经")) {
                flag = true;
            }
            if (!flag) {
        ...
        result = result.replace("礼拜", "星期");
        result = result.replaceAll("([偏|晚].*1点)|([明|下|周].见)|((?<!\\\\d)((\\\\d){1,2}(\\s)?-(\\s)?(\\\\d){1,2})[k|月])|五休二|5休2|((0\\d{2,3}-\\d{7,8})|(1[3584]\\d{9}))|早上好|上午好|下午好|晚上好|早日|((?<=\\d)点?到(上午|下午)?[\\d|;|:]*)", "");
        result = result.replaceAll("(?<=\\\\d)\\\\.(?=(00|30))", ":");
        String ruleB = "(?<!\\\\d)((\\\\d){2,})[分|秒]";
        Pattern patternB = Pattern.compile(ruleB);
        Boolean flagB = patternB.matcher(result).find();
        if (flagB) {
            Matcher matcher = patternB.matcher(result);
            int count = Integer.parseInt(matcher.group(1));
            if (count >= 60) {
                result = result.replaceAll(ruleB,"");
            }
        }
        String ruleC = "(?<!\\.)(?<!\\:)(?<!\\:)(?<!\\-)(?<!\\-)(?<!\\\\d)(\\\\d+)\\-(\\\\d+)(?!\\\\d)(?!\\-)(?!\\:)(?!\\.)";
        Pattern patternC = Pattern.compile(ruleC);
        Boolean flagC = patternC.matcher(result).find();
        if (flagC) {
            Matcher matcher = patternC.matcher(result);
            int count = Integer.parseInt(matcher.group(1));
            if (count > 12) {
                result = result.replaceAll(ruleC, "");
            }
        }
        result = result.replace("个","");
        result = result.replace("中旬","15号");
        result = result.replace("傍晚","午后");
        result = result.replace("大年","");
        result = result.replace("五一","劳动节");
        result = result.replace("白天","早上");
    ...
    public static void main(String args[]) {
        String text = "我明天下午五点半";
        TimeNormalizer normalizer = new TimeNormalizer();
        text = filterUnstandard(text);
        normalizer.parse(text,"2018-4-20-14-45-50");//可以设置基准时间，也可以不设置
        TimeUnit[] unit = normalizer.getTimeUnit();
        List<String> result = new ArrayList<>();
        for (TimeUnit timeUnit : unit) {
            result.add(CommonDateUtil.formatDate(timeUnit.getTime(), "yyyy-MM-dd HH:mm:ss") + "-" + timeUnit.getIsAllDayTime());//没有时分秒的标志
        }
        System.out.println(result);

        //考虑节日
        String text1 = "劳动节下午5：30";
        TimeNormalizer normalizer1 = new TimeNormalizer();
        text1 = filterUnstandard(text1);
        normalizer1.parse(text1,"2018-4-20-14-45-50");//可以设置基准时间，也可以不设置
        TimeUnit[] unit1 = normalizer1.getTimeUnit();
        List<String> result1 = new ArrayList<>();
        for (TimeUnit timeUnit : unit1) {
            result1.add(CommonDateUtil.formatDate(timeUnit.getTime(), "yyyy-MM-dd HH:mm:ss") + "-" + timeUnit.getIsAllDayTime());//没有时分秒的标志
        }
